        ...
        with_mock_app(mock_data, app) do
          ...
          if addon_data = get_mock_addon(mock_data, addon)
            ...
            if app_addon_data = get_mock_app_addon(mock_data, app, addon)
              ...
              {
                :body   => Heroku::OkJson.encode({'error' => "The add-on #{addon} is not installed for this app. Did you mean:\n\t#{addon_names.join("\n\t")}"}),
                :status => 422
              }
            end
          else
            ...
            {
              :body   => Heroku::OkJson.encode({'error' => "Could not find add-on #{addon}. Did you mean:\n\t#{addon_names.join("\n\t")}"}),
              :status => 422
            }
          end
        end
          ...
          if addon_data = get_mock_addon(mock_data, addon)
            ...
            unless app_addon_type_data = mock_data[:addons][app].detect {|data| data['name'] =~ %r{^#{addon.split(':').first}}}
              ...
              unless app_addon_data = get_mock_app_addon(mock_data, app, addon)
                ...
                # addon is currently installed
                {
                  :body   => Heroku::OkJson.encode({'error' => "Add-on already installed."}),
                  :status => 422
                }
              end
            else
              ...
              {
                :body   => Heroku::OkJson.encode({'error' => "#{app_addon_type_data['name']} add-on already added.\nTo upgrade, use addons:upgrade instead.\n"}),
                :status => 422
              }
            end
          else
            # addon does not exist
            {
              :body   => Heroku::OkJson.encode({'error' => "Add-on not found."}),
              :status => 422
            }
          end
        end
