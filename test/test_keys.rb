require "#{File.dirname(__FILE__)}/test_helper"

class TestKeys < MiniTest::Unit::TestCase
  KEY_DATA = "ssh-rsa  username@hostname.local"

  def test_delete_key_key_not_found
    assert_raises(Excon::Errors::NotFound) do
      heroku.delete_key(random_name)
    end
  end

  def test_delete_key
    heroku.post_key(KEY_DATA)
    response = heroku.delete_key('username@hostname.local')

    assert_equal(200, response.status)
  end

  def test_delete_keys
    response = heroku.delete_keys

    assert_equal(200, response.status)
  end

  def test_get_keys
    response = heroku.get_keys

    assert_equal(200, response.status)
    assert_equal([], response.body)
  end

  def test_post_key
    response = heroku.post_key(KEY_DATA)

    assert_equal(200, response.status)

    heroku.delete_key('username@hostname.local')
  end

end
