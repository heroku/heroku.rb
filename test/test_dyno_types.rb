...
class TestProcesses < MiniTest::Unit::TestCase
  ...
  def test_put_dyno_types_app_not_found
    assert_raises(Heroku::API::Errors::NotFound) do
      heroku.put_dyno_types(random_name, [{'name' => 'web', 'quantity' => 2}])
    end
  end

  def test_put_dyno_types_scale_down
    with_app('stack' => 'cedar') do |app_data|
      heroku.put_dyno_types(app_data['name'], [{'name' => 'web', 'quantity' => 2}])
      response = heroku.put_dyno_types(app_data['name'], [{'name' => 'web', 'quantity' => 1}])
  ...
  def test_put_dyno_types
    with_app('stack' => 'cedar') do |app_data|
      response = heroku.put_dyno_types(app_data['name'], [{'name' => 'web', 'quantity' => 2}])
